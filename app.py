...
# 🔥 التحديث الجذري لدالة تنظيف الصوت 🔥
def clean_text_for_audio(text):
    # 1. إزالة أوامر LaTeX الخاصة بالتنسيق
    text = re.sub(r'\\underline\{(.*?)\}', r'\1', text) # يزيل \underline{كلمة} ويبقي "كلمة"
    text = re.sub(r'\\textbf\{(.*?)\}', r'\1', text)    # يزيل \textbf
    text = re.sub(r'\\textit\{(.*?)\}', r'\1', text)    # يزيل \textit
    text = re.sub(r'\\begin\{.*?\}', '', text)
    text = re.sub(r'\\end\{.*?\}', '', text)
    text = re.sub(r'\\item', '', text)
    
    # 2. إزالة الرموز المزعجة
    text = text.replace('*', '')  # النجوم
    text = text.replace('#', '')  # الشبابيك
    text = text.replace('_', ' ') # يحول الشرطة السفلية لمسافة حتى لا ينطقها "Underscore"
    text = text.replace('-', '')
    text = text.replace('`', '')
...
# --- شاشة الدخول (باستخدام Form لمنع الفصل) ---
