"""
AI Science Tutor Pro - Revised and Hardened
Features added / improvements:
- Secrets moved to `st.secrets` usage throughout.
- Robust, pluggable model interface with multi-provider fallbacks (Google Generative AI, OpenAI if present).
- Safer and synchronous-friendly TTS handling using asyncio.run wrapper.
- Fixed streaming text display using placeholders (compatible with Streamlit).
- Improved Google Sheets/Drive handling with retries and local fallback logging.
- Session expiration and rate-limiting per-session.
- Persistent chat history (local JSON + optional Google Sheets backup).
- Teacher dashboard tools: view/export logs, adjust XP, broadcast messages.
- MCQ improvements: multi-choice generation, automatic checking, hints, explanations, XP rewards.
- Image handling: safe preview + send a short image-description prompt to the model.
- Defensive programming and structured logging.
- Comments and messages primarily in Arabic for ease of maintenance.
"""
...
# Expected entries in streamlit secrets:
# - TEACHER_MASTER_KEY
# - DRIVE_FOLDER_ID (optional)
# - GOOGLE_API_KEYS (list)
# - OPENAI_API_KEY (optional)
# - gcp_service_account (dict) (optional)
...
RATE_LIMIT_MIN_SECONDS = int(st.secrets.get("RATE_LIMIT_MIN_SECONDS", 1))  # per-user delay between requests
...
# Local backup files (if Google not configured)
...
def safe_write_local_log(entry: Dict[str, Any]):
    # Append JSON line for resiliency
    try:
        with open(LOCAL_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")
    except Exception as e:
        logger.exception("Failed to write local log: %s", e)
...
# - These functions try to use google APIs when available,
#   otherwise fallback to local files and logs.
# ==========================================
...
# - The function `call_model` unifies calls to different providers
# - It will attempt Google Generative AI (if configured), then OpenAI (if key present)
# - Returns text or raises Exception
# ==========================================
def init_google_genai_if_available():
    if genai and GOOGLE_API_KEYS:
        # try to pick a key and configure the library
        # NOTE: `genai.configure(api_key=...)` may be the right call depending on version
...
def call_model(prompt: str, *, model_preferences: Optional[List[str]] = None, max_output_chars: int = 4000) -> str:
    """
    Unified model call. Tries Google Generative AI (if genai available),
    then OpenAI (if openai and key available). Raises Exception on failure.
    """
    # Defensive sanitize
    if not prompt:
        return ""
    ...
    if genai and _GOOGLE_INITIALIZED:
        # Attempt multiple possible APIs to be compatible with versions.
        try:
            # Some versions use genai.generate_text or genai.TextGenerationModel
            # We'll try common patterns.
            if hasattr(genai, "TextGenerationModel"):
                # Newer client model class
                for candidate in (model_preferences or ["gemini-2.5-flash", "gemini-flash-latest", "gemini-pro-latest", "gemini-2.0-flash"]):
                    try:
                        ...
                        logger.debug("Google candidate %s failed: %s", candidate, e)
                # fallback - try a generic generate_text
        ...
        try:
            ...
            if hasattr(openai, "ChatCompletion"):
                resp = openai.ChatCompletion.create(
                    model="gpt-4o" if "gpt-4" in OPENAI_API_KEY.lower() else "gpt-3.5-turbo",
...
# ==========================================
# Speech-to-text helper (optional)
# ==========================================
...
def stream_text_to_placeholder(text: str, placeholder, delay: float = 0.02):
    ...
    for ch in text:
        buf += ch
        # update every few chars for efficiency
        if len(buf) % 20 == 0:
...
def log_login(user_name: str, user_type: str, details: str):
    ...
    # also attempt to append to sheets (background)
...
def update_xp(user_name: str, points: int):
    ...
    # background update to sheet/local
...
# - Supports text, image, and MCQ generation/checking modes
# - Produces Graphviz DOT blocks if model returns ```dot ... ```
# - Streams display and optionally plays TTS
# ==========================================
def process_ai_response(user_text: Any, input_type: str = "text"):
    ...
    ref = st.session_state.get("ref_text", "")[:20000]  # avoid extremely long context
...
"""
    ...
    try:
        if input_type == "image":
            # user_text expected: [caption, PIL.Image instance] or file bytes
            caption = "Please explain the image and highlight key science concepts."  # default
            image_obj = None
            if isinstance(user_text, list) and len(user_text) >= 2 and isinstance(user_text[1], Image.Image):
                image_obj = user_text[1]
                # create a small representation to send as base64 if user wants rich prompt
                buffered = BytesIO()
                ...
                # Append a hint that image is available as base64 (provider may or may not accept it)
            ...
            # user_text expected: dict with keys 'question' and 'answer'
            ...
            # default text or voice
            prompt = base_prompt + f"\nStudent: {user_text}\nAnswer:"
            # For longer answers, request step-by-step
            raw = call_model(prompt)
            # Strip dot diagrams
            dot_code = None
            if "```dot" in raw:
...
with st.sidebar:
    ...
    if st.session_state.user_type == "student":
        ...
        # Try to show top leaderboard from sheet or local
        try:
            client = get_gspread_client()
            if client:
                wb = client.open("App_Control")
                try:
                    ...
                    if not df.empty:
                        ...
                        st.text("لا يوج�� بيانات بعد")
                except Exception:
                    logger.debug("Gamification sheet read failed")
            else:
                # local fallback: read local logs and compute xp sums (best-effort)
        ...
        if st.button("تصدير السجلات كملف"):
            # Return raw local log file as download
            if os.path.exists(LOCAL_LOG_FILE):
        ...
        if st.button("بث"):
            # In a real setup this would write to a control sheet or notify clients via websocket.
    ...
    if DRIVE_FOLDER_ID and build and GCP_SA:
        svc = None
        try:
            svc = get_gspread_client()  # reuse cached auth (helper)
            # We only list files when asked to reduce API calls
            if st.button("تحميل قائمة ملفات Drive"):
                # Basic Drive read via googleapiclient
...
st.markdown("---")
st.caption("AI Science Tutor Pro — نسخة محسنة. الرجاء حفظ المفاتيح الحساسة في `st.secrets`.")

# Ensure graceful shutdown saving session chat history
try:
    # persist in-memory chat_history to local on every run
    if st.session_state.get("chat_history"):
        # simple transfer to local file
