...
# 🔥 دالة تحميل الموديل الآمنة (تم إصلاحها بالكامل) 🔥
@st.cache_resource
def load_ai_model():
    try:
        api_key = None
...
# محاولة تحميل الموديل عند البدء
try:
    model = load_ai_model()
except:
    model = None
...
def safe_generate_content(model, prompt):
    if not model:
        raise Exception("AI Model not connected")
