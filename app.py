...
# =========================
# Maps (Elementary Only as requested)
# =========================
STAGES = ["الابتدائية"]
GRADES = {
...
def get_llm(temp: float = 0.2):
    ...
    if not k:
        return None
    # Fix 404 error: Use correct model name
...
def sidebar():
    ...
    with st.sidebar:
        ...
        # 1. اختيار المرحلة بأمان
        try:
            stage_idx = STAGES.index(u["stage"])
        except ValueError:
            stage_idx = 0
        stage = st.selectbox("المرحلة", STAGES, index=stage_idx)

        # 2. اختيار الصف (مع حماية من الانهيار)
        available_grades = GRADES.get(stage, [])
        current_grade = u.get("grade")
        
        if current_grade in available_grades:
            grade_idx = available_grades.index(current_grade)
        else:
            grade_idx = 0
            
        grade = st.selectbox("الصف", available_grades, index=grade_idx)

        # 3. اختيار الترم
        try:
            term_idx = TERMS.index(u["term"])
        except ValueError:
            term_idx = 0
        term = st.selectbox("الترم", TERMS, index=term_idx)

        # 4. اختيار المادة
        avail_subjects = subjects_for(stage, grade)
        subject = st.selectbox("المادة", avail_subjects, index=0)

        # 5. اختيار اللغة
        try:
            lang_idx = LANGS.index(u["lang"])
        except ValueError:
            lang_idx = 0
        lang_ui = st.selectbox("اللغة", LANGS, index=lang_idx)
