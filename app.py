...
# 🔥 دالة المعالجة المركزية (لمنع التكرار) 🔥
def process_ai_response(user_text, input_type="text"):
    ...
    try:
        model = load_ai_model() # استدعاء الموديل
...
# 2. رسم الهيدر دائماً (ليظهر في البداية)
draw_header()

# 3. شاشة الدخول (داخل الحاوية الرئيسية)
...
with tab1:
    ...
    if audio_in: 
        if audio_in['bytes'] != st.session_state.last_audio_bytes:
            ...
            voice_code = get_voice_config(st.session_state.language)[1]
...
with tab4:
    ...
    with col_q1:
        if st.button("🎲 Single Question", use_container_width=True):
            ...
            {source}
            Constraint: Strictly from source. No LaTeX in text.
